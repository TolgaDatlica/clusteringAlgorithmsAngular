...
import { GoogleLocationPageService } from './googlelocation.service';
...
@Component({
  selector: 'googlelocation',
  styleUrls: ['googlelocation.component.scss'],
  templateUrl: './googlelocation.component.html',
})
export class GoogleLocationPageComponent implements OnInit, AfterViewInit {
  public graph = {
    data: [],
  ...
  */
  ...
  wifilist = [];
  constructor(private mainPageService: GoogleLocationPageService, private renderer: Renderer2, private elementRef: ElementRef,
              private sampleService: GoogleLocationPageService) {
  }
  ngAfterViewInit() {
  }
  ngOnInit() {
    ...
    try {
      ...
      parsedJSON.WIFI.forEach(element => {
        const object = {
          ...
          signalStrength: parseInt(element.RSSI, 10)
        };
