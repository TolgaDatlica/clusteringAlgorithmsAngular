...
import { SpatioTemporalService } from './spatiotemporal.service';
...
@Component({
  selector: 'spatiotemporal',
  styleUrls: ['spatiotemporal.component.scss'],
  templateUrl: './spatiotemporal.component.html',
})
export class SpatioTemporalPageComponent implements OnInit, AfterViewInit {
  public graph = {
    data: [],
  ...
  */
  ...
  locationlist = [];
  constructor(private mainPageService: SpatioTemporalService, private renderer: Renderer2, private elementRef: ElementRef,
    private sampleService: SpatioTemporalService) {
  }
  ngAfterViewInit() {
  }
  ngOnInit() {
  ...
  submit() {
    try {
      ...
      parsedJSON.Location.forEach(element => {
        ...
        let hours = 0;
        let minutes = 0;
        const normalizedDateTime: Date = new Date(0);
        if (element.Timestamp) {
          hours = (new Date(parseInt(element.Timestamp, 10) * 1000)).getHours();
          normalizedDateTime.setHours(hours);
          minutes = (new Date(parseInt(element.Timestamp, 10) * 1000)).getMinutes();
          normalizedDateTime.setMinutes(minutes);
        }
        this.locationlist.push({
          lat: parseFloat(element.Lat),
          lon: parseFloat(element.Lon),
          timestamp: Math.round(normalizedDateTime.getTime() / 1000),
          mac: counter,
  ...
  initMap() {
    ...
    this.DrawHeatmap(this.locationlist);
  ...
  graphCreate() {
    ...
    this.locationlist.forEach(element => {
      if (usedElements.filter(x => x.name == element.clusterNumber).length > 0) {
        ...
        findedElement.z.push(element.timestamp);
        findedElement.text.push((new Date(element.timestamp * 1000)).getHours());
      } else {
        usedElements.push(
          {
            x: [element.lat],
            ...
            mode: 'markers+text',
            type: 'scatter3d',
            name: (new Date(element.timestamp * 1000)).getHours(),
            text: [(new Date(element.timestamp * 1000)).getHours()],
